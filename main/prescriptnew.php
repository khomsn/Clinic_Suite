...
$filter = mysqli_query($link, "select * from drug_id WHERE typen!='Treatment' AND $cat AND $fdout ORDER BY `dgname` ASC");

if ($_POST['todo'] == 'กรอง' ) 
{
	if($_POST['type'] != '' AND $_POST['group'] !='' )
	{
		$filter = mysqli_query($link, "select * from drug_id WHERE typen='$_POST[type]' AND $cat AND $fdout AND  `groupn` ='$_POST[group]' ORDER BY `dgname` ASC");	
	}	
	if($_POST['type'] != '' AND $_POST['group'] =='' )
	{
		$filter = mysqli_query($link, "select * from drug_id WHERE typen='$_POST[type]'  AND $cat AND $fdout ORDER BY `dgname` ASC");	
	}	
	if($_POST['group'] !=''  AND  $_POST['type'] == '' )
	{
		$filter = mysqli_query($link, "select * from drug_id WHERE  `groupn` ='$_POST[group]'  AND $cat AND $fdout ORDER BY `dgname` ASC");	
	}	
	if($_POST['dgname1'] != '')
	{
		$filter = mysqli_query($link, "select * from drug_id WHERE dgname='$_POST[dgname1]'  AND $cat AND $fdout ORDER BY `dgname` ASC");	
	}	
...
?>
<div id="content">
<form method="post" action="prescriptnew.php" name="regForm" id="regForm">
